return function()
    ...
    vim.api.nvim_create_user_command("NeotestRunCurrent", function()
        neotest.run.run()
    end, { desc = "[r]un [t]est under [c]ursor in Neotest." })
    vim.api.nvim_create_user_command("NeotestRunDirectory", function()
        neotest.run.run(vim.fn.getcwd())
    end, { desc = "[r]un [t]est [d]irectory in Neotest." })
    vim.api.nvim_create_user_command("NeotestRunFile", function()
        neotest.run.run(vim.fn.expand("%"))
    end, { desc = "[r]un [t]est [f]ile in Neotest." })
