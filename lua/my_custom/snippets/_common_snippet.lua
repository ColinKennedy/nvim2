...
function M.append_parentheses_onto_line()
    return function_(function(_, _)
        local current_row = vim.fn.line(".") -- This is "1-or-greater"
        local current_line = vim.fn.getline(current_row)
        local current_buffer = 0

        vim.schedule(function()
            vim.api.nvim_buf_set_lines(current_buffer, current_row - 1, current_row, true, { current_line .. ")" })
        end)
    end)
end
...
M.print_snippet = snippet({
    ...
    trig = "p",
}, { text("print("), index(1), M.append_parentheses_onto_line() }, { show_condition = is_source_beginning("p") })
